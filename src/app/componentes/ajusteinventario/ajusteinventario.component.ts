...
public guardarMedicamentoAjusteInventario(itemt: any): boolean {
  ...
  // 2. Preparación de valores numéricos
  const ajuste = Number(itemt.cantidadAjuste) || 0;
  const sistema = Number(itemt.cantidadSistema) || 0;
  const fisica = Number(itemt.cantidadFisica) || 0;

  // 3. Validación de consistencia lógica
  if ((sistema + ajuste) !== fisica) {
    Swal.fire({
      icon: 'error',
      title: 'Cantidades inconsistentes',
      text: 'La sumatoria entre la cantidad en sistema y el ajuste debe ser igual a la cantidad física.',
    });
    itemt.editing = true; // Mantenemos el modo edición por el error
    return false;
  }

  // 4. Si la validación pasa, procedemos a actualizar o agregar
  itemt.editing = false;

  if (this.existeItem(itemt.idMedicamento)) {
    this.actualizarCantidad(itemt.idMedicamento, ajuste);
  } else {
  ...
  create() {
    if (this.generalForm.valid) {
      if (this.ajusteInventario.itemsAjuste.length > 0) {
        ...
        const tieneErrores = this.listaItems.some((itemAjuste: { cantidadSistema: number; cantidadFisica: number; cantidadAjuste: number; cantidad: number; }) => {
          ...
          const cantidadSistema = Number(itemAjuste.cantidadSistema) || 0;
          const cantidadFisica = Number(itemAjuste.cantidadFisica) || 0;
          const nuevacantidadSistema = cantidadSistema + cantidadAjuste; // Aquí calculamos el valor nuevo que quedaria   
          if (nuevacantidadSistema !== cantidadFisica) {
            Swal.fire({
              icon: 'error',
              title: 'Cantidades inconsistentes',
              text: 'La orden de ajuste tiene items donde la difrencia entre la cantidad en sistema y la cantidad fisica son inconsistentes.',
            });
            return true; // Detener la iteración si se encuentra un error
          }
          return false;
